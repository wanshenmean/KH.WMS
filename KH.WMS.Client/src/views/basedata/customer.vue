...
const crudApi = useCrudApi('customer')

const extraActionButtons = [
  {
    label: (row) => row.status === 1 ? '禁用' : '启用',
    type: 'warning',
    permission: 'bd:customer:toggle',
    show: (row) => true,
...
// ==================== 扩展字段 ====================
const { loadExtConfig, mergedColumns, mergedTableColumns, extractAndCleanExtData, withFlatExtLoad } = useExtFields('/api/customer/form-config')

const searchColumns = [
  { prop: 'customerCode', label: '客户编码', type: 'input', clearable: true, placeholder: '请输入客户编码' },
  { prop: 'customerName', label: '客户名称', type: 'input', clearable: true, placeholder: '请输入客户名称' },
  {
...
const searchModel = reactive({ customerCode: '', customerName: '', status: '' })

// ==================== 表格列 ====================
const baseTableColumns = [
  { prop: 'customerCode', label: '客户编码', width: 140 },
...
const baseFormColumns = [
  { prop: 'customerCode', label: '客户编码', type: 'input', required: true, placeholder: '请输入客户编码' },
  { prop: 'customerName', label: '客户名称', type: 'input', required: true, placeholder: '请输入客户名称' },
