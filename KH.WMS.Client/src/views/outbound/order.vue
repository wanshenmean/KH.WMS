<template>
  <div style="height: 100%; display: flex; flex-direction: column;">
    <KhPage ref="pageRef" title="出库单管理" module="outbound-order" :search-columns="searchColumns"
      ...
      :permission-prefix="'out:order'" :delete-show="(row) => row.allowedActions?.includes('DELETE')"
      :action-buttons="actionButtons" :detail-lines="detailLineConfigs" :detail-width="'1100px'"
    ...
    <!-- 新增/编辑弹窗 -->
    <KhDialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增出库单' : '编辑出库单'" width="1100px"
...
import { createOutboundOrder, updateOutboundOrder, getOutboundOrderDetail, allocateOutboundOrder, confirmOutboundOrder } from '@/api/outbound'

const pageRef = ref(null)
const crudApi = useCrudApi('outbound-order')
...
} = useExtFields('/api/outbound-order/form-config')
...
// ==================== 搜索 ====================
const searchColumns = [
  { prop: 'orderNo', label: '出库单号', type: 'input', clearable: true },
  {
    prop: 'orderType', label: '出库类型', type: 'select', clearable: true,
    options: 'dict:outbound_order_type',
  },
  {
    prop: 'orderStatus', label: '状态', type: 'select', clearable: true,
    options: 'dict:outbound_order_status',
  },
]
...
const statusTagMap = {
  ...
  RELEASING: { label: '释放中', type: 'warning' },
  RELEASED: { label: '已释放', type: '' },
...
const baseTableColumns = [
  ...
  {
    prop: 'orderType', label: '出库类型', width: 120, align: 'center',
    type: 'tag', tagMap: 'dict:outbound_order_type',
  },
  {
    prop: 'orderStatus', label: '状态', width: 100, align: 'center',
    type: 'slot',
  },
  { prop: 'warehouseId', label: '仓库', width: 100, align: 'center', type: 'tag', tagMap: 'dict:warehouse_list' },
  { prop: 'customerId', label: '客户', width: 100, align: 'center' },
...
const detailLineConfigs = [
  {
    ...
    columns: [
      ...
      { prop: 'shippedQty', label: '已发数量', width: 120, align: 'right' },
...
const actionButtons = [
  ...
  },
  {
    label: '确认',
    type: 'primary',
    permission: 'out:order:confirm',
    show: (row) => row.allowedActions?.includes('CONFIRM'),
    confirm: '确认后不可修改和删除，是否继续？',
    onClick: async (row) => {
      try {
        const res = await confirmOutboundOrder(row.id)
        KhMessageFn.success(res.message || '确认成功')
  ...
  },
  {
    label: '分配',
    type: 'warning',
    permission: 'out:order:allocate',
    show: (row) => row.allowedActions?.includes('ALLOCATE'),
    confirm: '确认执行库存分配？',
    onClick: async (row) => {
      try {
        const res = await allocateOutboundOrder(row.id)
        KhMessageFn.success(res.message || '分配成功')
        pageRef.value?.reload()
      } catch {
        // request.js 已处理错误提示
      }
...
const baseFormColumns = [
  { prop: 'orderNo', label: '出库单号', type: 'input', disabled: true, colSpan: 2, placeholder: '自动生成' },
  {
    prop: 'orderType', label: '出库类型', type: 'select', required: true, colSpan: 2,
    options: 'dict:outbound_order_type',
  },
  {
  ...
  {
    prop: 'customerId', label: '客户', type: 'select', clearable: true,
    options: 'dict:customer_list', placeholder: '请选择客户',
  },
  {
...
const baseLineColumns = [
  { prop: 'materialCode', label: '物料编码', type: 'select', width: 200, placeholder: '物料编码', options: 'dict:material_list' },
  { prop: 'orderedQty', label: '订单数量', type: 'number', width: 200, min: 0, precision: 2, controls: false, placeholder: '数量', align: 'right' },
  { prop: 'batchNo', label: '批次号', type: 'input', width: 200, placeholder: '批次号' },
  { prop: 'remark', label: '备注', type: 'input', placeholder: '备注' },
]
...
const handleUpdate = async (row) => {
  ...
  resetForm()
  try {
    const res = await getOutboundOrderDetail(row.id)
...
const handleSubmit = async () => {
  ...
  try {
    if (dialogMode.value === 'create') {
      ...
      await createOutboundOrder(requestBody)
      KhMessageFn.success('新增成功')
    } else {
      await updateOutboundOrder(requestBody.order.id, requestBody)
