...
async function pageTitle(relative: string) {
  ...
  return frontmatterTitle ?? source.match(/^#\s+(.+)$/m)?.[1]?.replace(/`/g, '') ?? path.basename(relative, '.md');
}

async function items(files: string[]) {
  return Promise.all(files.map(async (relative) => ({ text: await pageTitle(relative), link: toUrl(relative) })));
}

const architectureItems = await items(architectureFiles);
const frontendItems = await items(frontendFiles);
const backendTutorialItems = await items(backendTutorialFiles);
...
const conceptItems = await Promise.all(conceptGroups.map(async (group) => ({
  text: group.text,
  collapsed: true,
  ...
  themeConfig: {
    ...
    sidebar: {
      '/backend/': [
        ...
        { text: '培训与考核', collapsed: true, items: [...trainingItems, { text: 'PPT / Word 下载', link: '/training-materials' }] },
