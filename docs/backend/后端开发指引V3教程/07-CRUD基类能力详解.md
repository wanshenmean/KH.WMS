---
title: "第 7 章 CRUD 基类能力详解 教程"
description: "第 7 章 CRUD 基类能力详解 教程：说明适用场景、当前实现、设计边界与开发或排障入口。"
status: current
...
第一次完整实现主从表时，直接按[KH.WMS 主从表页面配置与开发实战指引](/backend/KH.WMS主从表页面配置与开发实战指引)完成建表、Navigate、事务保存、页面配置和验收；本节只解释 CRUD 基类中的接入边界。
