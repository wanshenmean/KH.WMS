...
主从页面没有“一个配置项生成全部功能”的入口。当前职责边界是：

- `tableColumns` 配置主列表。
- `formColumns` 配置主表表单。
- `lineColumns` 配置 `KhEditableTable` 的可编辑明细。
- `detailLines` 只配置“查看”弹窗里的只读明细，不参与新增、编辑和保存。

标准组合为 `KhPage + KhDialog + KhForm + KhEditableTable`。关闭 `KhPage` 内置新增、编辑；编辑时先加载详情并保留已有明细 ID；更新时提交当前完整明细集合。

不同业务的接口契约不能混用：入库单继续使用自己的 `{ order, lines }` DTO；通用培训案例直接提交主实体及 `items`。

从建表、Navigate、事务保存、页面配置到新增/编辑/删除验收的逐步教程，请阅读：[KH.WMS 主从表页面配置与开发实战指引](/backend/KH.WMS主从表页面配置与开发实战指引)。
