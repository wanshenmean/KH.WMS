...
/// <summary>
/// 单据扩展字段契约实现
/// 负责：查询配置、转换前端格式、ExtData JSON 序列化/反序列化
/// </summary>
[RegisteredService(Lifetime = ServiceLifetime.Scoped, ServiceType = typeof(ICfgDocumentFieldExtContract))]
public class CfgDocumentFieldExtContract(
    IRepository<CfgDocumentType, long> docTypeRepository,
    IRepository<CfgDocumentField, long> docFieldRepository,
    Core.Caching.ICacheService cache) : ICfgDocumentFieldExtContract
{
    private const string CACHE_PREFIX = CacheConstants.Data.PREFIX;

    /// <inheritdoc />
    public async Task<List<CfgDocumentField>> GetFieldsAsync(string docTypeCode, string fieldLevel = "HEADER")
    {
        var cacheKey = $"{CACHE_PREFIX}{docTypeCode}:{fieldLevel}";
        if (cache.TryGet<List<CfgDocumentField>>(cacheKey, out var cached))
            return cached!;

        var docType = await docTypeRepository.GetFirstOrDefaultAsync(d => d.TypeCode == docTypeCode);
    ...
    /// <inheritdoc />
    public List<Dictionary<string, object?>> BuildFormColumns(List<CfgDocumentField> fields)
    {
    ...
    /// <inheritdoc />
    public string? SerializeExtData(Dictionary<string, object?> allData, List<CfgDocumentField> fields)
    {
    ...
    /// <inheritdoc />
    public async Task<Dictionary<string, object?>> DeserializeProcessableExtDataAsync(string docTypeCode, string? extDataJson, string fieldLevel = "HEADER")
    {
        var allData = DeserializeExtData(extDataJson);
        if (allData.Count == 0) return allData;

        var fields = await GetFieldsAsync(docTypeCode, fieldLevel);
    ...
    /// <inheritdoc />
    public void ClearCache(string docTypeCode)
    {
        cache.Remove($"{CACHE_PREFIX}{docTypeCode}:HEADER");
        cache.Remove($"{CACHE_PREFIX}{docTypeCode}:LINE");
    }
