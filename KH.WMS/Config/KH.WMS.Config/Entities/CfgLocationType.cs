...
namespace KH.WMS.Config.Abstractions
{
    /// <summary>
    /// 库位类型
    /// 定义不同库位的用途分类（如普通库位、暂存库位、质检库位等）
    /// </summary>
    [ConfigDb]
    [SugarTable("cfg_location_type")]
    [SugarIndex("uk_type_code", nameof(TypeCode), OrderByType.Asc)]
    public class CfgLocationType : BaseEntity<long>, IEnableDisableEntity
    {
        /// <summary>
        /// 类型编码（如 NORMAL、STAGING、PICKING、QC、REJECT）
        /// </summary>
